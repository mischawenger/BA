...
where $o_{i}$ stands for the zone prediction result of the $i$-th individual machine learning algorithm. This leads to $O$ being a set of $^rP_m$ permutations (repetitions allowed), whith $r$ being the number of zones and m the number of different machine learning algorithms. The machine learning methods have to be conditionally independent for $y_{t} \in O$ as the random variable of the observations in time $t$.
\item A quadratic matrix $B$ holding the emission probabilities of observation liklelihoods:
\begin{equation*}
B = 
\begin{pmatrix}
